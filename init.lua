...
do
  ...
  require('telescope').setup {
    ...
    pickers = {
      ...
      live_grep = {
        additional_args = function(_) return { '--follow', '--hidden', '--glob', '!**/.git*/*' } end,
      },
    },
    extensions = {
      ...
      file_browser = {
        ...
        -- NOTE: Neotree netrw hijack must be disabled
        hijack_netrw = true,
      },
    },
  }
  ...
  vim.keymap.set('n', '<leader>sm', '<cmd>Telescope noice<cr>', { desc = '[S]earch Notification [M]essages' })
  vim.keymap.set('n', '<leader>sb', '<cmd>Telescope file_browser<CR>', { desc = '[S]earch with File [B]rowser' })
...
do
  ...
  local servers = {
    ...
    dprint = {
      ...
      root_markers = { 'dprint.json', 'dprint.jsonc' },
      workspace_required = true,
    },
  ...
  -- Merge two tables
  --
  -- Source - https://stackoverflow.com/a/71433446
  -- Posted by Heechul Ryu, modified by community.
  ...
  local extra_servers = { marksman = {}, nixd = {} }

  for name, server in pairs(merge(extra_servers, servers)) do
...
do
  ...
  local parsers = { 'bash', 'c', 'diff', 'html', 'lua', 'luadoc', 'markdown', 'markdown_inline', 'regex', 'query', 'vim', 'vimdoc' }
